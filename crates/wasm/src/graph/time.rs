...
mod tests {
    ...
    #[test]
    fn roundtrip_recent() {
        let ms = 1_779_431_805_882i64;
        let s = format_iso(ms);
        let back = parse_iso_ms(&s).unwrap();
        assert_eq!(back / 1000, ms / 1000);
    }

    #[test]
    fn parses_y2k() {
        let s = "2000-01-01T00:00:00.000Z";
        let ms = parse_iso_ms(s).unwrap();
        assert_eq!(format_iso(ms), s);
    }
}
