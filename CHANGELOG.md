...
### Fixed
- **npm / bunx distribution repaired end-to-end** — `release.yml` removed and folded into `publish.yml` as a build+upload matrix that runs inside the same workflow that cuts the tag. `gh release create` via GITHUB_TOKEN does NOT trigger downstream `release:published` events, which is why every prior release (v0.1.29 … v0.1.32) shipped with zero binary assets and every `bunx rs-learn` install silently failed the 404 download. The new layout: `version` job bumps + tags + releases → `build` matrix uploads six-platform binaries → `registries` job publishes to crates.io + npm. Binaries are guaranteed to exist before npm latest points at the version.
- **Version-resolution loop fixed** — prior logic bumped on every workflow_run even when caught up, causing infinite version-bump cycles. Now bumps only when `tag v$current` already exists AND `published > current`.
- **npm wrapper self-heals on first run** — `npm/install.js` extracted as shared module with retry+backoff download; `npm/postinstall.js` pre-fetches but failure no longer fatal; `npm/bin/rs-learn` (and new `rs-learn-validate`) lazy-download the binary on first invocation if postinstall was skipped (bunx `--ignore-scripts`, offline install, etc.). Binary now lives in `npm/vendor/` (separate from wrapper scripts in `npm/bin/`) so wrapper can't be clobbered by the download.
- **npm package hygiene** — `files` whitelist restricts tarball to `bin/`, `install.js`, `postinstall.js`, `README.md`; repository URL normalized to `git+https://...`; `rs-learn-validate` bin registered.
- **crates.io publish now strict** — previously silently skipped when `CARGO_REGISTRY_TOKEN` unset, masking the fact that the crate was never registered. Now hard-fails the job so missing secrets surface immediately.
- **.gitattributes** added to pin LF endings on shell/node wrapper scripts so Windows-checked-in files remain executable on Unix.
