...
use super::time::{format_iso, parse_iso_ms};
...
impl Ingestor {
    ...
    pub async fn add_episode(
        ...
        let before_ms = reference_time.and_then(parse_iso_ms);
        let previous = self
    ...
    async fn load_previous_episodes_before(&self, before_ms: Option<i64>, limit: usize) -> Result<Value> {
        let mut rows = match before_ms {
            Some(t) => self.store.conn.query(
                "SELECT id, content, source, created_at FROM episodes WHERE created_at < ?1 ORDER BY created_at DESC LIMIT ?2",
                libsql::params![t, limit as i64],
            ).await?,
