...
impl Store {
    ...
    pub async fn update_reasoning_success_rate(&self, id: &str, new_rate: f64) -> Result<()> {
        self.conn.execute(
            "UPDATE reasoning_bank SET success_rate=?1 WHERE id=?2",
            libsql::params![new_rate, id.to_string()],
        ).await?;
        Ok(())
    }

    pub async fn get_reasoning_success_rate(&self, id: &str) -> Result<Option<f64>> {
        let mut rows = self.conn.query(
            "SELECT success_rate FROM reasoning_bank WHERE id=?1",
            libsql::params![id.to_string()],
        ).await?;
        Ok(match rows.next().await? {
            Some(row) => Some(row.get::<f64>(0).unwrap_or(0.0)),
            None => None,
        })
    }
